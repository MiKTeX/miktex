...
#if defined(MIKTEX) && defined(MIKTEXPOPTAPI)
MIKTEXPOPTAPI(poptContext) poptGetContext(const char * name, int argc, const char ** argv,
#else
...
/*@=sizeoftype@*/
	...
	if (opt->arg && poptArgType(opt) == POPT_ARG_NONE) {
		...
		con->os->nextCharArg = NULL;
#if defined(MIKTEX)
		} else if (opt->argInfo & POPT_ARGFLAG_OPTIONAL) {
		con->os->nextArg = NULL;
#endif
	    } else {
...
#if defined(MIKTEX) && defined(MIKTEXPOPTAPI)
MIKTEXPOPTAPI(int) poptAddAlias(poptContext con, struct poptAlias alias,
#else
