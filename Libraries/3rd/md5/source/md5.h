...
#if defined(MIKTEX)
MIKTEXMD5API(void) md5_init(md5_state_t *pms);
#else
...
#if defined(MIKTEX)
MIKTEXMD5API(void) md5_append(md5_state_t *pms, const md5_byte_t *data, int nbytes);
#else
...
#if defined(MIKTEX)
MIKTEXMD5API(void) md5_finish(md5_state_t *pms, md5_byte_t digest[16]);
#else
