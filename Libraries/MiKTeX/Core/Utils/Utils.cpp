...
/**
 * @brief Get the relativized path from the given path and root
 *
 * This function takes a path and a root path as input and returns the
 * relativized path from the given path with respect to the root path. The
 * relativized path is the path that is relative to the root path, without
 * including the root path itself.
 *
 * @param lpszPath The path for which the relativized path needs to be
 * calculated.
 * @param lpszRoot The root path against which the relativized path needs to be
 * calculated.
 * @return const char* The relativized path from the given path and root.
 */
...
bool Utils::ParseDvipsFontMapLine(const string& line, DvipsFontMapEntry& mapEntry)
{
    ...
    for (string::const_iterator it = line.begin(); it != line.end(); ++it)
    {
        ...
        else if (*it == '<')
        {
            ...
            else if (fileName.HasExtension(".pfa") || fileName.HasExtension(".pfb"))
            {
