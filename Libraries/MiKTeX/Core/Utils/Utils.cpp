...
tuple<string, string, string, string> Utils::ParseLocaleIdentifier(const string& localeIdentifier)
{
  ...
  for (char& ch : language)
  {
    ...
    {
      MIKTEX_FATAL_ERROR_2("Invalid locale identifier.", "localeIdentifier", localeIdentifier);
    }
  }
  if (pos == string::npos)
  {
...
vector<string> Utils::GetUILanguages()
{
  auto session = SessionImpl::TryGetSession();
  if (session != nullptr)
  {
  ...
  for (const auto& envName : { "LC_ALL", "LC_MESSAGES", "LANG" })
  {
    ...
    if (::GetEnvironmentString(envName, lang) && !lang.empty())
    {
      ...
      {
        result.push_back(fmt::format("{0}_{1}", language, country));
      }      
      break;
    }
  }
