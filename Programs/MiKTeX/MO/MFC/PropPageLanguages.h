/* PropPageLanguages.h:                                 -*- C++ -*-

   Copyright (C) 2000-2016 Christian Schenk

   This file is part of MiKTeX Options.

   MiKTeX Options is free software; you can redistribute it and/or
   modify it under the terms of the GNU General Public License as
   published by the Free Software Foundation; either version 2, or (at
   your option) any later version.

   MiKTeX Options is distributed in the hope that it will be useful,
   but WITHOUT ANY WARRANTY; without even the implied warranty of
   MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
   General Public License for more details.

   You should have received a copy of the GNU General Public License
   along with MiKTeX Options; if not, write to the Free Software
   Foundation, 59 Temple Place - Suite 330, Boston, MA 02111-1307,
   USA. */

#if defined(_MSC_VER)
#  pragma once
#endif

class PropPageLanguages : public CPropertyPage
{
public:
  PropPageLanguages(std::shared_ptr<MiKTeX::Packages::PackageManager> pManager);

public:
  ~PropPageLanguages();

private:
  enum { IDD = IDD_PROPPAGE_LANG };

private:
  CListCtrl listControl;

protected:
  virtual BOOL OnInitDialog();

protected:
  virtual BOOL OnApply();

protected:
  virtual void DoDataExchange(CDataExchange * pDX);

protected:
  afx_msg BOOL OnHelpInfo(HELPINFO * pHelpInfo);

protected:
  afx_msg void OnContextMenu(CWnd * pWnd, CPoint point);

protected:
  afx_msg void OnDblclkList(NMHDR * pNMHDR, LRESULT * pResult);

protected:
  afx_msg void OnItemChanged(NMHDR * pNMHDR, LRESULT * pResult);

protected:
  afx_msg void OnItemChanging(NMHDR * pNMHDR, LRESULT * pResult);

protected:
  DECLARE_MESSAGE_MAP();

private:
  struct MyLanguageInfo : public LanguageInfo
  {
    MyLanguageInfo(const LanguageInfo & languageInfo) :
      LanguageInfo(languageInfo)
    {
    }
    bool loaderExists = false;
    PathName loaderPath;
    bool active = false;
    bool newActive = false;
    vector<string> packageNames;
  };

private:
  vector<MyLanguageInfo> languages;

private:
  void ReadLanguageDat();

private:
  void Refresh();

private:
  int GetSelectedItem();

private:
  void EnableButtons();

private:
  void RefreshRow(int);

private:
  void InsertLanguage(int idx);

private:
  void InsertColumn(int colIdx, const char * lpszLabel, const char * lpszLongest);

private:
  void SetElevationRequired(bool f);

private:
  void SetChanged(bool f);

private:
  vector<string> WhichPackage(const char * lpszTeXInputFile);

private:
  bool InstallLanguagePackages(const vector<string> toBeInstalled);

private:
  bool inserting = false;

private:
  PathName languageDat;

private:
  std::shared_ptr<MiKTeX::Packages::PackageManager> pManager;

private:
  std::shared_ptr<MiKTeX::Core::Session> session = MiKTeX::Core::Session::Get();
};
