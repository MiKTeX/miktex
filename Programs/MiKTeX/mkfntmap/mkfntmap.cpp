...
void MakeFontMapApp::WriteMap(StreamWriter& writer, const set<FontMapEntry>& set1)
{
  ...
  {
    writer.Write(fmt::format("{} {}", fme.texName, fme.psName));
    if (!fme.specialInstructions.empty())
    {
      writer.Write(fmt::format(" \" {} \"", fme.specialInstructions));
    }
    for (Tokenizer tok(fme.headerList, ";"); tok; ++tok)
    {
      writer.Write(fmt::format(" {}", *tok));
    }
    writer.WriteLine();
  }
}
...
void MakeFontMapApp::WriteDvipdfmMap(StreamWriter& writer, const set<FontMapEntry>& set1)
{
  for (const FontMapEntry& fme : set1)
  {
    ...
    if (!field2.empty())
    {
      writer.Write(fmt::format(" {}", field2));
    }
    else if (!field3.empty())
    {
      writer.Write(fmt::format(" {}", "default"));
    }
    if (!field3.empty())
    {
      writer.Write(fmt::format(" {}", field3));
    }
    if (!options.empty())
    {
